...
await mkdir(path.join(appDir, "node_modules", "@leichtgewicht"), { recursive: true });
...
await Promise.all([
  ...
  ...["bonjour-service", "multicast-dns", "dns-packet", "fast-deep-equal", "thunky"].map((name) => cp(path.join(root, "node_modules", name), path.join(appDir, "node_modules", name), { recursive: true })),
  cp(path.join(root, "node_modules", "@leichtgewicht", "ip-codec"), path.join(appDir, "node_modules", "@leichtgewicht", "ip-codec"), { recursive: true }),
